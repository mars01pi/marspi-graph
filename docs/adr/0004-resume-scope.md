...
3. CodingLoop and Supervisor are **invoke-oriented**: they are designed to
   run to completion in one process. Cross-process or post-crash Resume
   re-enters the node with a **fresh** agent conversation; prior
   implementer/worker turns are not restored.
...
- HITL that only needs graph State + interrupt payload is honest today.
